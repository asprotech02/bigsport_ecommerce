            ...
            <div class="d-flex flex-column flex-md-row justify-content-between align-items-md-end mb-4 border-bottom pb-4 gap-3">
                ...
                <div>
                    @php
                        ...
                        // Tangkap parameter Brand
                        $brandParam = request('brand');
                        $activeBrand = (is_array($brandParam) && count($brandParam) == 1) ? $brandParam[0] : (is_string($brandParam) ? $brandParam : null);

                        $currentUrl = route('product.index');
                        $breadcrumbs = [];

                        // 2. Bangun Breadcrumb beserta URL-nya (Logika Diperketat)
                        
                        // JIKA URL MENGANDUNG ?type=...
                        if ($activeType == 'sale') {
                            $breadcrumbs[] = ['label' => 'SALE', 'url' => $currentUrl . '?type=sale'];
                        } elseif ($activeType == 'new') {
                            $breadcrumbs[] = ['label' => 'PRODUK BARU', 'url' => $currentUrl . '?type=new'];
                        } elseif ($activeType == 'featured') {
                            $breadcrumbs[] = ['label' => 'PRODUK EKSKLUSIF', 'url' => $currentUrl . '?type=featured'];
                        } elseif ($activeType == 'brand') {
                            // 🌟 INI PENANGKAP UTAMA UNTUK URL ?type=brand
                            $breadcrumbs[] = ['label' => 'BRAND', 'url' => $currentUrl . '?type=brand'];
                        } 
                        // JIKA URL MENGANDUNG ?brand[]=Nike (Pilih logo dari beranda)
                        elseif ($activeBrand) {
                            $breadcrumbs[] = ['label' => 'BRAND', 'url' => $currentUrl . '?type=brand'];
                            $breadcrumbs[] = ['label' => strtoupper($activeBrand), 'url' => $currentUrl . '?brand[]=' . urlencode($activeBrand)];
                        } 
                        // JIKA URL MENGANDUNG KATEGORI/GENDER
                        else {
                            if ($activeGen) {
                                $breadcrumbs[] = ['label' => 'KATEGORI ' . strtoupper($activeGen), 'url' => $currentUrl . '?gender=' . urlencode($activeGen)];
                            }
                            if ($activeCat) {
                                $catUrl = $currentUrl . '?';
                        ...
                        // Jika tetap kosong (Berada di halaman Semua Produk polosan)
